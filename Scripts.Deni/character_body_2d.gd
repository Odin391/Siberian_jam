...
func _physics_process(delta):
	...
	# Если не активен и не говорит, можно проверять игрока на взаимодействие
	if not is_talking:
...
func start_dialogue_sequence():
	...
	if label:
		label.visible = true
		label.text = "А, вот и наш горе-гость... Ты, наверное, удивлён, 
		почему вместо офиса оказался
		 в этом латте-аду."
	 ...
	 Теперь твой путь лежит через эту равнину никто еще непроходил дальше :"
	...
	# После окончания всех фраз - начать движение
	is_talking = false
	is_active = true
