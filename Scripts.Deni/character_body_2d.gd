...
func start_dialogue_sequence():
	...
	if label:
		label.visible = true
		label.text = "А, вот и наш горе-гость... Ты, наверное, удивлён, почему вместо офиса оказался в этом латте-аду."
		await get_tree().create_timer(10.0).timeout
		
		# Вторая фраза
		label.text = "Знаешь, Лате и Лаванда — дамы с характером. Они не прощают неуважения. Пролить латте на лаванду — это всё равно что плюнуть в душу утра. Теперь твой путь лежит через эту равнину — никто ещё не проходил дальше :"
		await get_tree().create_timer(16.0).timeout
		
		# Третья фраза
		label.text = "Мне пора на варку. А ты ступай к реке — там твоё первое испытание. И помни: уважение к мелочам возвращает домой."
